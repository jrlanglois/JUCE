...
bool WavAudioFormat::replaceMetadataInFile (const File& wavFile, const StringPairArray& newMetadata)
{
    ...
    auto reader = rawToUniquePtr (static_cast<WavAudioFormatReader*> (createReaderFor (wavFile.createInputStream().release(), true)));
...
private:
    ...
    MemoryBlock writeToBlock (WavAudioFormat& format, const StringMap& meta)
    {
...

        {
            ...
            auto stream = std::make_unique<MemoryOutputStream> (mb, false);
            auto writer = format.createWriterFor (stream, writerOptions);
